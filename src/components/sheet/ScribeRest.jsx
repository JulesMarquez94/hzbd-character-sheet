 ...
 *
 * ------------------------------------------------------------ one ink, not two
 * This window served **both** kinds of leaf until 2026-09-10, on an `ephemeral`
 * flag: the same shelf and the same words at two prices, one capped at two and
 * paid for in Supplies, the other capped by an attribute and free.
 *
 * The fading half is not written at a rest any more. It is a card you play, for
 * Action Points and Willpower, and it has a window of its own on the quick bar
 * because a leaf thrown down in a fight is one leaf and not a list of them: see
 * EphemeralWindow.jsx. So everything here is the permanent kind, and the flag is
 * gone with the branch it carried.
 *
 ...
 */
export default function ScribeRest({ character, kind, state, draft, onDraft, onClose }) {
  ...
  const rows = scribeRows(draft, state);
  const spent = scribingCost(draft, state);
  const ink = scribingWillpower(draft, state);
  ...
  return (
      ...
      footer={
        <>
          <span className="brew-step-note">
            {rows.length} of {room} · {spent} Supplies
            {ink > 0 ? ` · ${ink} Willpower` : ''}
          </span>
    ...
    >
      <p className="frame-foot" style={{ marginTop: 0 }}>
        {room} {room === 1 ? 'leaf' : 'leaves'} a night, and each one costs its Quartz out of the
        crate: 50 Supplies at Novice, 150 at Adept and 200 at Master. They go into your pack,
        ready to be clipped to your belt.
      ...
      <section className="brew-step">
        <div className="brew-step-head">
          <span className="brew-step-label">On the desk</span>
          <span className={`brew-step-note${rows.length === 0 ? ' is-open' : ''}`}>
            {rows.length === 0 ? 'Empty' : `${rows.length} of ${room} · ${spent} Supplies`}
          </span>
        </div>
          ...
          <div className="rest-labours">
            {rows.map((row) => (
              <Leaf
                ...
                onWord={(wordId) => onDraft(toggleScribeWord(draft, state, row.index, wordId))}
                onDrop={() => {
                  onDraft(dropScribe(draft, state, row.index));
                  setOpen(null);
                }}
      ...
      <div className="card-brief-wall">
        {shelf.map((card) => {
          ...
          const canPay = scribingAffordable(character, kind, draft, state, card);
          ...
          return (
            ...
            >
              ...
              <Gated
                ...
                onClick={() => onDraft(addScribe(draft, state, card.id))}
                why={
                  full
                ...
                title={allowed ? `Write ${card.name} for ${price} Supplies` : undefined}
              >
 ...
 */
function Leaf({ row, state, open, onOpen, onWord, onDrop, onRead }) {
    ...
    <div className={`rest-labour${open ? ' is-chosen' : ''}`}>
      ...
      >
        ...
        <span className="rest-labour-line">
          {row.tier} · {row.supplies} Supplies
          {row.words.length > 0
 ...
 * `tag.label` the moment anybody types in the box, so searching the desk's own
 * shelf took the sheet down on the first keystroke. Found on 2026-09-10, in a
 * browser pass on the window next door, and fixed the same way there and in
 * ScrollWindow: `poolTags` is the helper that returns the `{ id, label, kind }`
 * the row wants. It also sorts the rungs up the ladder before the schools and
 * drops a tag every card carries, since a chip that selects the whole shelf
 * narrows nothing. The `{ card }` wrapper is the shape it takes, being written
 * for a pool of options.
 */
