...
export function LoadoutChooser({ talent, character, state, readOnly, onToggle, onClear, onClose }) {
  ...
  return (
    ...
    >
      ...
      {offered.length === 0 ? (
        <p className="pick-notice is-warning">
          This build&rsquo;s codex holds no {spec.school} {plural(spec.noun, 2)} this rank can take
          yet. Add them to the codex and they appear here on their own.
        </p>
      ) : (
        later > 0 && (
          <p className="pick-line">
            {later} more {plural(spec.noun, later)} in this school {later === 1 ? 'is' : 'are'} held
            back for higher ranks.
          </p>
        )
      )}
...
function LoadoutBrowser({ talent, rank, spec, opened = [], character, onClose }) {
  ...
  return (
    ...
    >
      ...
      <SubSchoolWall options={visible} noun={spec.noun} character={character} />
    </Modal>
  );
}
...
/**
 * The pool as a wall, cut into its sub-schools.
 *
 ...
 * one of them is left as a plain wall, because a single heading over everything
 * is a heading that says nothing.
 */
function SubSchoolWall({ options, noun, character, action = null }) {
  const stack = useCardStack();
  const groups = bySubSchool(options);
...
/**
 * The options cut by sub-school and named alphabetically, with anything the
 * codex left unfiled gathered at the end. Order inside a group is the order the
 * pool arrived in: what this rank can take first, then by name.
 */
function bySubSchool(options) {
  const groups = new Map();
  for (const option of options) {
    const key = option.sub ?? 'Unfiled';
...
function poolTags(options) {
  const seen = new Set();
  for (const option of options) {
    for (const tag of option.card.tags ?? []) seen.add(tag);
  }
